    ...
    try {
      ...
      if (snapshot.exists && snapshot.value != null) {
        ...
        if (!widget.isAddMode && selectedId != null) {
          if (_userNameController.text.isEmpty && usersMap.containsKey(selectedId)) {
  ...
  Future<void> _updateNotification() async {
    ...
    try {
      ...
      if (_selectedUserId == 'all_users') {
        ...
        debugPrint('Notification will be ${isAddMode ? "saved" : "updated"} in database only');
      }
      ...
      if (isAddMode) {
        ...
        debugPrint('BEFORE SAVE - _selectedUserId is null: ${_selectedUserId == null}');
        debugPrint('BEFORE SAVE - _selectedUserId isEmpty: ${_selectedUserId?.isEmpty ?? 'N/A'}');
        final newData = {
        ...
        debugPrint('BEFORE UPDATE - _selectedUserId is null: ${_selectedUserId == null}');
        final updatedData = {
          'notificationTitle': _titleController.text.trim(),
          'notificationMessage': _messageController.text.trim(),
          'userId': _selectedUserId!,
      ...
      if (notificationSent) {
      ...
      } else if (_hasFCMToken || _manualFCMToken != null) {
        successMessage = isAddMode
        ...
        debugPrint('WARNING: Notification ${isAddMode ? "created" : "updated"} but FCM push failed');
      } else {
        successMessage = isAddMode
        ...
        duration = const Duration(seconds: 3);
        debugPrint('INFO: Notification ${isAddMode ? "created" : "updated"} but no FCM token available');
      }
