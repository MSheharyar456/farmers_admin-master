...
class _SoldPostsContentState extends State<SoldPostsContent> {
  ...
  List<Post> get _filteredPosts {
    // First filter only sold posts that are approved, then apply other filters
    final soldPosts = _posts.where((post) => post.postIsSold == true).toList();
    final filtered = soldPosts.where(_matchesFilters).toList();
      ...
      newRows.add(
        PlutoRow(
          cells: {
            ...
            'category': PlutoCell(value: post.postCategory), // Keep raw value
  ...
  Widget _buildEmptyState() {
    // If source data is empty, show full empty state (No Data)
    if (_posts.isEmpty) {
      return Center(
        child: Padding(
    ...
    // If data exists but filters result in no matches (No Results)
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Filters Section - Responsive Layout with APPLY BUTTON FILTERING
                    isMobile
                        ? Column(
                            children: [
                              TextField(
